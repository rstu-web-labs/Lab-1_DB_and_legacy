...
CREATE TABLE car_shop.strana (
    id SERIAL PRIMARY KEY, 
    strana VARCHAR(20) not null unique
);

CREATE TABLE car_shop.brand (
    id SERIAL PRIMARY KEY, 
    brand VARCHAR(20) not null,
    id_strana int references car_shop.strana(id) on delete restrict
);
...
insert into car_shop.strana (strana)
	select brand_origin 
...
insert into car_shop.brand (brand, id_strana)
	select 
	    substring(auto from 1 for position(' ' in auto) - 1) as brand, 
	    strana.id
	from raw_dataa.sales
	left join car_shop.strana  ON strana.strana = raw_dataa.sales.brand_origin
	group by brand, strana.id;
...
left join car_shop.Cars c on c.id = com.id_Cars 
left join car_shop.brand br on br.id =c.id_brand 
left join car_shop.strana st on st.id = br.id_strana
group by brand_origin
having strana is not null;
