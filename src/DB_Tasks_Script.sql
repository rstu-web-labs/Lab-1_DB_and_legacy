...
create table car_shop.car(
	...
	id_automobileModel int references automobileModel(id)on delete restrict not null,
...
insert into automobileModel (automobileModel)
	select
...
insert into car (id_brand, id_automobileModel, gasoline_consumption)
	select 
		brand.id as b, 
	...
	left join automobileModel on automobileModel.automobileModel = substring(raw_data.sales.auto from position(' ' in raw_data.sales.auto) + 1 for position(',' in raw_data.sales.auto) - position(' ' in raw_data.sales.auto) - 1) 
...
insert into equipment (id_car, id_color)
	...
	left join car on concat((select brand from brand where brand.id = car.id_brand), (select automobileModel from automobileModel where automobileModel.id = car.id_automobileModel)) = concat(substring(raw_data.sales.auto from 1 for position(' ' in raw_data.sales.auto) - 1),substring(raw_data.sales.auto from position(' ' in raw_data.sales.auto) + 1 for position(',' in raw_data.sales.auto) - position(' ' in raw_data.sales.auto) - 1))
	group by c, r;
...
left join automobileautomobileModel m on m.id =c.id_automobileModel 
group by person
order by person;
