...
auto varchar(30),
gasoline_consumption varchar(30),
price float,
date date,
...
create table model (
	...
	gas_consumption float check (gas_consumption > 0), -- просто число с плавающей точкой (дано изначально в исходном файле); P.S. отел юзать тип DECIMAL с параметрами (2, 2), но не срослось, не хотело работать
...
create table sales(
	...
	price float check (price >= 0), -- цена - используем число с плавающей точкой; P.S. хотел использовать тип MONEY, но он жёстко и жестоко ругался при импорте данных
...
select distinct split_part(split_part(auto, ', ', 1), ' ', 1) as brand_name,
				coalesce (country.id, (select id from country where name = 'Unknown')) as country_id
from raw_data.sales
