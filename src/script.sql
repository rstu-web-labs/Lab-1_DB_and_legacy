...
COPY raw_data.sales FROM '/Users/vaclav/Desktop/backend/Lab-1_DB_and_legacy/cars.csv' WITH CSV HEADER NULL 'null';
...
insert into car_shop.car (id_brand, id_model, gasoline_consumption)
	select 
		brand.id as b, 
		model.id as m1,  
...
insert into car_shop.equipment (id_car, id_color)
	...
	left join car_shop.car on concat((select brand from brand where brand.id = car.id_brand), (select model from model where model.id = car.id_model)) = concat(substring(raw_data.sales.auto from 1 for position(' ' in raw_data.sales.auto) - 1),substring(raw_data.sales.auto from position(' ' in raw_data.sales.auto) + 1 for position(',' in raw_data.sales.auto) - position(' ' in raw_data.sales.auto) - 1))
	group by c, r;
...
left join car_shop.model m on m.id =c.id_model
group by person
order by person;
