...
from 'D:\cars.csv' delimiter ',' csv header null 'null';
...
select substring(auto from position(' ' in auto) + 1 for position(',' in auto) - position(' ' in auto) - 1) as model
from raw_data.sales
group by model;
...
where phone like '+1%'nt on equipment.id_color = color.id and equipment.id_car = car.id
