...
copy raw_data.sales (id,auto,gasoline_consumption,price,date,person_name,phone,discount,brand_origin) from 'D:\\cars.csv' delimiter ',' csv header null 'null';
...
create table car (
    ...
    model varchar(255),
    gasoline_consumption numeric(3,1) check (gasoline_consumption > 0),
    id_brand int references brand(id_brand),
    id_color int references color(id_color),
    constraint fk_brand_id foreign key (id_brand) references brand(id_brand),
    constraint fk_color_id foreign key (id_color) references color(id_color)
);
...
create table sales (
    ...
    constraint fk_car_id foreign key (id_car) references car(id_car),
