...
CREATE TABLE raw_data.sales (
    ...
    auto VARCHAR(50),
    gasoline_consumption DECIMAL(3,1), --двузначное число, может иметь 1 знак после ,
    price DECIMAL(19,12),
    	--По условию цена цена не может быть больше семизначной суммы, а в данных есть числа с 12 знаками после запятой 11243.440000000002
    	--Поэтому тип данных DECIMAL(19,12)
    date DATE,
    person_name VARCHAR(70), --тут хранится фио, выделим больше места
    phone VARCHAR(50),
    discount DECIMAL(4,2), --скидка двузначное число, может иметь 2 знака после запятой
...
CREATE TABLE car_shop.person (
 id serial PRIMARY KEY, --id покупателя. Инкремент
 person_name VARCHAR(70) not NULL, --имя покупателя из сырого
 phone VARCHAR(50) NOT null --телефон из сырого
);

CREATE TABLE car_shop.spr_country (
 id serial PRIMARY KEY, --id страны производителя. Инкремент
 country_name VARCHAR(50) unique NOT null --назв страны. Может быть ноль (из исходника)
);

CREATE TABLE car_shop.brand (
 id serial PRIMARY KEY, --id бренда авто. Инкремент
 id_country int, -- ВК связь со справочником стран
 name_brand VARCHAR(50) not null check (name_brand NOT LIKE ''), --назв бренда. Одно слово
 FOREIGN KEY (id_country) REFERENCES car_shop.spr_country(id)
);

CREATE TABLE car_shop.model (
 id serial PRIMARY KEY, --id модели авто. Инкремент
 id_brand int, -- ВК связь с брендом
 name_model VARCHAR(50) not null, --назв модели
 gasoline_consumption DECIMAL(3,1) CHECK (gasoline_consumption >= 0 AND gasoline_consumption <= 99.9) DEFAULT null,
 			--установлен предел значений и дефолтное значение null. Относится к таблице модели, тк не меняется для модели
 FOREIGN KEY (id_brand) REFERENCES car_shop.brand(id)
);

CREATE TABLE car_shop.sett (
 id serial PRIMARY KEY, --id комплектации. Инкремент
 id_model int, -- ВК связь с моделью
 color VARCHAR(50) not null, --цвет. Одно слово
 FOREIGN KEY (id_model) REFERENCES car_shop.model(id)
);


CREATE TABLE car_shop.sale (
 id serial PRIMARY KEY, --id . Инкремент
 id_person int, --ВК клиент
 id_sett int, -- ВК связь с комплектацией
 ...
 date DATE, -- дата сделки. Тип дата
...
--это было просто или я сделала не то?
