...
left join car_shop.color c2 on c2.color = substring(s.auto from ', (.*)')
left join car_shop.model m on concat((select brand from car_shop.brand b where b.id = m.id_brand),model_name) = concat(substring(s.auto from 1 for position(' ' in s.auto) - 1),substring(s.auto from position(' ' in s.auto) + 1 for position(',' in s.auto) - position(' ' in s.auto) - 1))
...
from raw_data.sales
left join car_shop.model on concat((select brand from car_shop.brand where brand.id = model.id_brand), model_name) = concat(substring(raw_data.sales.auto from 1 for position(' ' in raw_data.sales.auto) - 1),substring(raw_data.sales.auto from position(' ' in raw_data.sales.auto) + 1 for position(',' in raw_data.sales.auto) - position(' ' in raw_data.sales.auto) - 1))
...
JOIN car_shop.brand b ON m.id_brand = b.id
...
SELECT
car_shop.buyer.name AS person, 
    STRING_AGG(CONCAT(car_shop.brand.brand, ' ', car_shop.model.model_name), ', ') AS cars
FROM car_shop.sales  
JOIN car_shop.equipment on car_shop.sales.id_equipment = car_shop.equipment.id
JOIN car_shop.model ON car_shop.equipment.id_model = car_shop.model.id
JOIN car_shop.color ON car_shop.equipment.id_color = car_shop.color.id
JOIN car_shop.brand ON car_shop.model.id_brand = car_shop.brand.id
JOIN car_shop.buyer ON car_shop.sales.id_buyer = car_shop.buyer.id
GROUP BY car_shop.buyer.name
ORDER BY car_shop.buyer.name;
...
FROM 
  car_shop.sales s -- тут нужно было выбирать из ваших новых табличек
JOIN 
  car_shop.brand b ON b.brand = substring(s.auto from 1 for position(' ' in s.auto) - 1)
WHERE 
